...
public class ResponseWrapperMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        // Extract the "id" field from the response payload to build the message
        ...
        try
        {
            ...
            {
                orderId = idProp.GetString() ?? string.Empty;
            }
        }
        catch
        {
            // ignore parse errors — wrap anyway without id
        }

        var message = string.IsNullOrEmpty(orderId)
            ? $"Order {action} successfully"
            : $"Order \"{orderId}\" {action} successfully";
